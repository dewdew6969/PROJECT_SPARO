...
export default function MainTabNavigator() {
  const { t, language, profile, pendingMatchesCount, setPendingMatchesCount } = useAppStore();
    ...
    const checkPendingMatches = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const pendingCount = data.filter(m => {
            const isChallenger = m.challenger_id === profile?.id || m.sender_username === profile?.username;
            return m.status && m.status.toLowerCase() === 'pending' && !isChallenger;
          }).length;
        ...
        tabBarIcon: ({ color }) => {
          ...
          return (
            <View>
              ...
              {route.name === 'Matches' && pendingMatchesCount > 0 && (
                <View style={{
                  position: 'absolute',
                  top: -2,
                  right: -4,
